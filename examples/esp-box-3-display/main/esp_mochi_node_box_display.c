...
static const char *TAG = "esp_mochi_node_box_display";
static const char *DEFAULT_HEADING = "Mochi";
static const char *DEFAULT_TEXT = "Waiting for display.show from the Mochi gateway.";

esp_err_t esp_mochi_node_box_display_build_status_payload(
    const esp_mochi_node_box_display_t *display,
    char **out_payload_json)
{
    ...
    cJSON_AddNumberToObject(payload, "headingMaxLength", ESP_MOCHI_NODE_BOX_DISPLAY_MAX_HEADING_LEN);
    cJSON_AddNumberToObject(payload, "textMaxLength", ESP_MOCHI_NODE_BOX_DISPLAY_MAX_TEXT_LEN);
    cJSON_AddNumberToObject(payload, "width", BSP_LCD_H_RES);
    cJSON_AddNumberToObject(payload, "height", BSP_LCD_V_RES);

    return esp_mochi_node_example_take_json_payload(payload, out_payload_json);
}

static void apply_render_locked(esp_mochi_node_box_display_t *display)
{
    lv_label_set_text(display->heading_label, display->heading);
    lv_label_set_text(display->text_label, display->text);
}

esp_err_t esp_mochi_node_box_display_render(
...
static void create_display_ui_locked(esp_mochi_node_box_display_t *display)
{
...
esp_err_t esp_mochi_node_box_display_start(esp_mochi_node_box_display_t *display)
{
    if (display == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    return esp_mochi_node_box_display_render(display, DEFAULT_HEADING, DEFAULT_TEXT);
}
