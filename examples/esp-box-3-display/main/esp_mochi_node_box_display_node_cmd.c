...
#include "esp_mochi_node_box_display_node_cmd.h"
...
static const char *TAG = "esp_mochi_node_display_cmd";
    ...
    esp_mochi_node_error_t *out_error)
{
    ...
    esp_mochi_node_box_display_t *display = (esp_mochi_node_box_display_t *)context;
    const char *heading = NULL;
    const char *text = NULL;
    cJSON *params = NULL;

    esp_err_t err = esp_mochi_node_example_parse_json_params(params_json, &params, out_error);
    if (err != ESP_OK) {
    ...
    err = parse_required_text(
        params,
        "heading",
        ESP_MOCHI_NODE_BOX_DISPLAY_MAX_HEADING_LEN,
        &heading,
        out_error);
    if (err != ESP_OK) {
        cJSON_Delete(params);
    ...
    err = parse_required_text(
        params,
        "text",
        ESP_MOCHI_NODE_BOX_DISPLAY_MAX_TEXT_LEN,
        &text,
        out_error);
    if (err != ESP_OK) {
        cJSON_Delete(params);
    ...
    if (esp_mochi_node_box_display_render(display, heading, text) != ESP_OK) {
    ...
    cJSON_Delete(params);
    return esp_mochi_node_box_display_build_status_payload(display, out_payload_json);
}
    ...
    esp_mochi_node_box_display_t *display)
{
    ...
    static const esp_mochi_node_command_t DISPLAY_SHOW_COMMAND = {
        .name = "display.show",
        .handler = handle_display_show,
        .context = NULL,
    };
    static const esp_mochi_node_command_t DISPLAY_STATUS_COMMAND = {
        .name = "display.status",
        .handler = handle_display_status,
        .context = NULL,
    };

    esp_mochi_node_command_t show_command = DISPLAY_SHOW_COMMAND;
    show_command.context = display;
    esp_mochi_node_command_t status_command = DISPLAY_STATUS_COMMAND;
    status_command.context = display;

    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_capability(node, "display"),
        TAG,
        "registering display capability failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &show_command),
        TAG,
        "registering display.show failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &status_command),
        TAG,
