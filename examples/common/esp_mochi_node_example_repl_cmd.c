...
static const char *TAG = "esp_mochi_node_repl_cmd";
static esp_mochi_node_handle_t s_node;

#define ESP_MOCHI_NODE_REPL_WIFI_CONNECT_TIMEOUT_TICKS pdMS_TO_TICKS(30000)
...
static int wait_for_wifi_before_connect(const char *request_name)
{
    ...
    if (!esp_mochi_node_wifi_wait_for_connection(ESP_MOCHI_NODE_REPL_WIFI_CONNECT_TIMEOUT_TICKS)) {
    ...
    const char *request_name)
{
    ...
    esp_err_t err = esp_mochi_node_request_connect(s_node, request);
    if (err != ESP_OK) {
...
static int connect_secret_from_repl(
    ...
    esp_mochi_node_connect_source_t source,
    const char *secret,
...
static int disconnect_node_from_repl(void)
{
    ...
    esp_err_t err = esp_mochi_node_request_disconnect(s_node);
    if (err != ESP_OK) {
...
static int handle_status_command(int argc, char **argv)
{
    ...
    esp_mochi_node_wifi_status_t wifi_status = {0};
    esp_mochi_node_wifi_get_status(&wifi_status);

    printf("saved session available: %s\n", esp_mochi_node_has_saved_session(s_node) ? "yes" : "no");
...
static int handle_wifi_command(int argc, char **argv)
{
    if (argc >= 3 && strcmp(argv[1], "set") == 0) {
        ...
        esp_err_t err = esp_mochi_node_wifi_set_credentials(argv[2], passphrase);
        if (err != ESP_OK) {
    ...
    if (argc == 2 && strcmp(argv[1], "clear") == 0) {
        esp_err_t err = esp_mochi_node_wifi_clear_credentials();
        if (err != ESP_OK) {
    ...
    if (argc == 2 && strcmp(argv[1], "connect") == 0) {
        esp_err_t err = esp_mochi_node_wifi_connect();
        if (err != ESP_OK) {
    ...
    if (argc == 2 && strcmp(argv[1], "disconnect") == 0) {
        esp_err_t err = esp_mochi_node_wifi_disconnect();
        if (err != ESP_OK) {
