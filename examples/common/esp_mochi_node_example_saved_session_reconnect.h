 ...
 */
typedef struct {
    esp_mochi_node_handle_t node; /**< Node to reconnect. */
    TaskHandle_t task; /**< Background task that waits for Wi-Fi and reissues reconnects. */
} esp_mochi_node_example_saved_session_reconnect_t;
 ...
 *
 * Call this from the example's `esp_mochi_node_event_cb_t`. The helper retries
 * only retryable transport-loss outcomes and does not retry auth rejections.
 *
 * @param[in,out] state Helper state returned from
 *            @ref esp_mochi_node_example_saved_session_reconnect_start.
