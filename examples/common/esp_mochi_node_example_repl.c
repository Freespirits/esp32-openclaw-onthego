...
static const char *TAG = "esp_mochi_node_repl";
static esp_console_repl_t *s_repl;

esp_err_t esp_mochi_node_example_repl_start(esp_mochi_node_handle_t node)
{
    if (node == NULL) {
    ...
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_example_repl_register_commands(node),
        TAG,
        "registering REPL commands failed");

    esp_console_repl_config_t repl_config = ESP_CONSOLE_REPL_CONFIG_DEFAULT();
    repl_config.prompt = "mochi> ";
...
#else
#error Unsupported console transport for the Mochi REPL
#endif
