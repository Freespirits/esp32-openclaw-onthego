...
#include "esp_mochi_node_common_device_node_cmd.h"
...
#include "esp_timer.h"
#include "esp_wifi.h"
#include "esp_mochi_node_example_json.h"
#include "esp_mochi_node_wifi.h"

static const char *TAG = "esp_mochi_node_device_cmd";
...
{
    esp_mochi_node_wifi_status_t wifi = {0};
    esp_mochi_node_wifi_get_status(&wifi);
...
static cJSON *build_device_info_payload(esp_mochi_node_handle_t node)
{
    const char *device_id = esp_mochi_node_get_device_id(node);
...
static cJSON *build_device_status_payload(esp_mochi_node_handle_t node)
{
    cJSON *root = cJSON_CreateObject();
    ...
    cJSON_AddBoolToObject(root, "savedSessionAvailable", esp_mochi_node_has_saved_session(node));
    ...
    esp_mochi_node_error_t *out_error)
{
    ...
    (void)params_len;
    (void)out_error;
    return esp_mochi_node_example_take_json_payload(
...
esp_err_t esp_mochi_node_common_register_device_node_commands(esp_mochi_node_handle_t node)
{
    static const esp_mochi_node_command_t DEVICE_INFO_COMMAND = {
        .name = "device.info",
        .handler = handle_device_info,
        .context = NULL,
    };
    static const esp_mochi_node_command_t DEVICE_STATUS_COMMAND = {
        .name = "device.status",
        .handler = handle_device_status,
        .context = NULL,
    };
    static const esp_mochi_node_command_t WIFI_STATUS_COMMAND = {
        .name = "wifi.status",
    ...
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_capability(node, "device"),
        TAG,
        "registering device capability failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_capability(node, "wifi"),
        TAG,
        "registering wifi capability failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &DEVICE_INFO_COMMAND),
        TAG,
        "registering device.info failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &DEVICE_STATUS_COMMAND),
        TAG,
        "registering device.status failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &WIFI_STATUS_COMMAND),
        TAG,
