...
#include "esp_mochi_node_example_gpio_node_cmd.h"
...
static const char *TAG = "esp_mochi_node_gpio";
    ...
    esp_mochi_node_error_t *out_error)
{
    ...
    cJSON *params = NULL;
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_example_parse_json_params(params_json, &params, out_error),
        TAG,
        "invalid params");
...
esp_err_t esp_mochi_node_example_register_gpio_node_commands(esp_mochi_node_handle_t node)
{
    memset(s_gpio_mode_configured, 0, sizeof(s_gpio_mode_configured));
    memset(s_gpio_modes, 0, sizeof(s_gpio_modes));

    static const esp_mochi_node_command_t GPIO_MODE_COMMAND = {
        .name = "gpio.mode",
        .handler = handle_gpio_mode,
        .context = NULL,
    };
    static const esp_mochi_node_command_t GPIO_READ_COMMAND = {
        .name = "gpio.read",
        .handler = handle_gpio_read,
        .context = NULL,
    };
    static const esp_mochi_node_command_t GPIO_WRITE_COMMAND = {
        .name = "gpio.write",
    ...
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_capability(node, "gpio"),
        TAG,
        "registering gpio capability failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &GPIO_MODE_COMMAND),
        TAG,
        "registering gpio.mode failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &GPIO_READ_COMMAND),
        TAG,
        "registering gpio.read failed");
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_register_command(node, &GPIO_WRITE_COMMAND),
        TAG,
