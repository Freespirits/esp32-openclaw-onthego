# esp_mochi_node_unity_tests

This is the ESP-IDF Unity test app for the `esp-mochi-node` component.

Files:

- `CMakeLists.txt`: test app project file
- `main/test_esp_mochi_node.c`: test cases
...
not require a live Mochi gateway.
...
```bash
idf.py -C components/esp-mochi-node/test_apps/esp_mochi_node_unity_tests set-target esp32s3
idf.py -C components/esp-mochi-node/test_apps/esp_mochi_node_unity_tests build
idf.py -C components/esp-mochi-node/test_apps/esp_mochi_node_unity_tests -p /dev/ttyACM0 flash monitor
```
