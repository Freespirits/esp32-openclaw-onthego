...
#include "nvs.h"
#include "nvs_flash.h"
#include "esp_mochi_node_identity.h"
#include "esp_mochi_node_internal.h"
#include "esp_mochi_node.h"
#include "esp_mochi_node_persisted_session.h"
#include "unity.h"

typedef struct {
    volatile bool seen;
    volatile esp_mochi_node_event_t event;
    volatile esp_err_t local_err;
    volatile int connected_count;
    volatile int connect_failed_count;
    volatile int disconnected_count;
    volatile esp_mochi_node_connect_failure_reason_t connect_failed_reason;
    volatile esp_mochi_node_disconnected_reason_t disconnected_reason;
...
static const esp_mochi_node_websocket_client_ops_t s_test_websocket_client_ops = {
...
const esp_mochi_node_websocket_client_ops_t *esp_mochi_node_test_websocket_client_ops(void)
{
    return &s_test_websocket_client_ops;
}

static void reset_mochi_storage(void)
{
    nvs_flash_deinit();
...
static esp_err_t request_connect_saved_session(esp_mochi_node_handle_t node)
{
    const esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_SAVED_SESSION,
        .gateway_uri = NULL,
        .value = NULL,
    };
    return esp_mochi_node_request_connect(node, &request);
}

static esp_err_t request_connect_setup_code(esp_mochi_node_handle_t node, const char *setup_code)
{
    const esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_SETUP_CODE,
        .gateway_uri = NULL,
        .value = setup_code,
    };
    return esp_mochi_node_request_connect(node, &request);
...
{
    const esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_TOKEN,
...
{
    const esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_PASSWORD,
        .gateway_uri = gateway_uri,
        .value = password,
    };
    return esp_mochi_node_request_connect(node, &request);
...
static esp_err_t request_connect_no_auth(esp_mochi_node_handle_t node, const char *gateway_uri)
{
    const esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_NO_AUTH,
        .gateway_uri = gateway_uri,
        .value = NULL,
    };
    return esp_mochi_node_request_connect(node, &request);
...
static void assert_persisted_session_empty(const esp_mochi_node_persisted_session_t *session)
{
    TEST_ASSERT_NOT_NULL(session);
    TEST_ASSERT_EQUAL_UINT8(0, session->version);
    TEST_ASSERT_NULL(session->gateway_uri);
    TEST_ASSERT_NULL(session->device_token);
    TEST_ASSERT_FALSE(esp_mochi_node_persisted_session_is_present(session));
}
    ...
    void *user_ctx)
{
    ...
    if (event == ESP_MOCHI_NODE_EVENT_CONNECTED) {
        s_event_recorder.connected_count++;
    } else if (event == ESP_MOCHI_NODE_EVENT_CONNECT_FAILED) {
        s_event_recorder.connect_failed_count++;
        const esp_mochi_node_connect_failed_event_t *failed = event_data;
        if (failed != NULL) {
            s_event_recorder.local_err = failed->local_err;
            s_event_recorder.connect_failed_reason = failed->reason;
        }
    } else if (event == ESP_MOCHI_NODE_EVENT_DISCONNECTED) {
        s_event_recorder.disconnected_count++;
        const esp_mochi_node_disconnected_event_t *disconnected = event_data;
...
static bool wait_for_event(esp_mochi_node_event_t expected, TickType_t timeout_ticks)
{
...
static esp_err_t blocking_command_handler(
    ...
    esp_mochi_node_error_t *out_error)
{
    (void)node;
    (void)params_json;
    (void)params_len;
...
static void destroy_with_pending_notification_task(void *arg)
{
    ...
    ctx->destroy_err = esp_mochi_node_destroy(ctx->node);
    ctx->remaining_notify_count = ulTaskNotifyTake(pdTRUE, 0);
    xTaskNotifyGive(ctx->completion_waiter);
    vTaskDelete(NULL);
}

TEST_CASE("persisted session stores loads and can be cleared by storing empty state", "[esp_mochi_node][session]")
{
    reset_mochi_storage();

    char gateway_uri[] = "wss://gateway.example/ws";
    char device_token[] = "device-token-123";
    esp_mochi_node_persisted_session_t session = {0};
    esp_mochi_node_persisted_session_t update = {
        .version = 1,
        .gateway_uri = gateway_uri,
        .device_token = device_token,
    };

    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_store(&session, &update));
    TEST_ASSERT_TRUE(esp_mochi_node_persisted_session_is_present(&session));
    ...
    esp_mochi_node_persisted_session_t loaded = {0};
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_load(&loaded));
    TEST_ASSERT_TRUE(esp_mochi_node_persisted_session_is_present(&loaded));
    ...
    esp_mochi_node_persisted_session_t cleared = {0};
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_store(&session, &cleared));
    assert_persisted_session_empty(&session);

    esp_mochi_node_persisted_session_free(&loaded);
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_load(&loaded));
    assert_persisted_session_empty(&loaded);

    esp_mochi_node_persisted_session_free(&loaded);
    esp_mochi_node_persisted_session_free(&session);
}

TEST_CASE("persisted session load ignores unsupported stored versions", "[esp_mochi_node][session]")
{
    reset_mochi_storage();

    nvs_handle_t nvs = 0;
    TEST_ASSERT_EQUAL(ESP_OK, nvs_open("mochi", NVS_READWRITE, &nvs));
    ...
    uint8_t version = 0;
    TEST_ASSERT_EQUAL(ESP_OK, nvs_open("mochi", NVS_READONLY, &nvs));
    TEST_ASSERT_EQUAL(ESP_ERR_NVS_NOT_FOUND, nvs_get_u8(nvs, "session_v", &version));
    nvs_close(nvs);

    esp_mochi_node_config_t config = {0};
    esp_mochi_node_config_init_default(&config);

    esp_mochi_node_handle_t node = NULL;
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_create(&config, &node));
    TEST_ASSERT_NOT_NULL(node);
    TEST_ASSERT_FALSE(esp_mochi_node_has_saved_session(node));
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_destroy(node));
}

TEST_CASE("persisted session load clears malformed stored state and node still boots", "[esp_mochi_node][session]")
{
    reset_mochi_storage();
...
TEST_CASE("node create deep-copies tls cert pem", "[esp_mochi_node][config]")
{
    reset_mochi_storage();
...
TEST_CASE("persisted session rejects partial state", "[esp_mochi_node][session]")
{
    reset_mochi_storage();
    ...
    char device_token[] = "device-token-123";
    esp_mochi_node_persisted_session_t session = {0};
    esp_mochi_node_persisted_session_t invalid = {
        .version = 1,
        .gateway_uri = gateway_uri,
        .device_token = NULL,
    };
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_mochi_node_persisted_session_store(&session, &invalid));
    assert_persisted_session_empty(&session);
...
TEST_CASE("persisted session store validates inputs before mutating state", "[esp_mochi_node][session]")
{
    reset_mochi_storage();
    ...
    esp_mochi_node_persisted_session_t session = {0};
    esp_mochi_node_persisted_session_t initial = {
        .version = 1,
        .gateway_uri = initial_gateway_uri,
        .device_token = initial_device_token,
    };
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_store(&session, &initial));

    char replacement_gateway_uri[] = "wss://replacement.example/ws";
    char replacement_device_token[] = "device-token-456";
    esp_mochi_node_persisted_session_t replacement = {
        .version = 1,
        .gateway_uri = replacement_gateway_uri,
        .device_token = replacement_device_token,
    };

    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_mochi_node_persisted_session_store(NULL, &replacement));
    TEST_ASSERT_EQUAL_STRING("wss://gateway.example/ws", session.gateway_uri);
    TEST_ASSERT_EQUAL_STRING("device-token-123", session.device_token);
    ...
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_mochi_node_persisted_session_store(&session, NULL));
    TEST_ASSERT_EQUAL_STRING("wss://gateway.example/ws", session.gateway_uri);
    TEST_ASSERT_EQUAL_STRING("device-token-123", session.device_token);
...
TEST_CASE("identity persists seed and auth payload signing", "[esp_mochi_node][identity]")
{
    reset_mochi_storage();

    uint8_t seed[ESP_MOCHI_NODE_ED25519_SEED_LEN] = {0};
    for (size_t i = 0; i < sizeof(seed); ++i) {
        seed[i] = (uint8_t)(i + 1U);
    }

    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_identity_store_seed_if_absent(seed, sizeof(seed)));

    esp_mochi_node_identity_t identity = {0};
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_identity_load_or_create(&identity));
    ...
    TEST_ASSERT_EQUAL(
        ESP_OK,
        esp_mochi_node_identity_build_auth_payload_v3(
            &identity,
            "node-host",
            "node",
            "node",
    ...
    char signature[ESP_MOCHI_NODE_SIGNATURE_B64_BUFFER_LEN] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        esp_mochi_node_identity_sign_payload(&identity, payload, signature, sizeof(signature)));
    TEST_ASSERT_TRUE(contains_only_base64url_chars(signature));

    esp_mochi_node_identity_t reloaded = {0};
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_identity_load_or_create(&reloaded));
    ...
    free(payload);
    esp_mochi_node_identity_free(&reloaded);
    esp_mochi_node_identity_free(&identity);
}

TEST_CASE("identity load recovers from malformed seed and clears saved session", "[esp_mochi_node][identity][session]")
{
    reset_mochi_storage();
    ...
    esp_mochi_node_persisted_session_t loaded_session = {0};
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_persisted_session_load(&loaded_session));
    assert_persisted_session_empty(&loaded_session);
    esp_mochi_node_persisted_session_free(&loaded_session);

    uint8_t stored_seed[ESP_MOCHI_NODE_ED25519_SEED_LEN] = {0};
    ...
    TEST_ASSERT_EQUAL_UINT32(ESP_MOCHI_NODE_ED25519_SEED_LEN, (uint32_t)stored_seed_len);
    nvs_close(nvs);
...
TEST_CASE("connect saved session without persisted session fails", "[esp_mochi_node][api]")
{
    reset_mochi_storage();
...
TEST_CASE("setup code rejects invalid or ambiguous auth shapes", "[esp_mochi_node][api]")
{
    reset_mochi_storage();
    reset_transport_state();
...
TEST_CASE("explicit connect request validates psk and no-auth arguments", "[esp_mochi_node][api]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    TEST_ASSERT_EQUAL(
        ESP_ERR_INVALID_ARG,
        esp_mochi_node_request_connect(
            node,
            &(esp_mochi_node_connect_request_t){
                .source = (esp_mochi_node_connect_source_t)99,
    ...
    TEST_ASSERT_EQUAL(
        ...
        request_connect_no_auth(
            node,
            ""));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE, esp_mochi_node_request_disconnect(node));

    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_destroy(node));
}

TEST_CASE("password auth is excluded from the signed device payload", "[esp_mochi_node][auth]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    const esp_mochi_node_connect_request_t password_request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_PASSWORD,
        .gateway_uri = "ws://gateway.example/ws",
        .value = "secret-password",
    };
    const esp_mochi_node_connect_request_t token_request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_TOKEN,
        .gateway_uri = "ws://gateway.example/ws",
        .value = "shared-token",
    };

    esp_mochi_node_connect_request_source_t password_source = {0};
    esp_mochi_node_connect_request_source_t token_source = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        esp_mochi_node_build_connect_source_from_request(
            &password_request,
            &password_source));
    TEST_ASSERT_EQUAL(
        ESP_OK,
    ...
    esp_mochi_node_connect_material_t password_material = {0};
    esp_mochi_node_connect_material_t token_material = {0};

    esp_mochi_node_lock_state(node);
    node->active_connect_source = password_source;
    memset(&password_source, 0, sizeof(password_source));
    TEST_ASSERT_EQUAL(
        ESP_OK,
        esp_mochi_node_resolve_active_connect_material_locked(
            node,
            &password_material));

    esp_mochi_node_clear_connect_source_struct(&node->active_connect_source);
    ...
    TEST_ASSERT_EQUAL(
        ...
        esp_mochi_node_resolve_active_connect_material_locked(
            node,
            &token_material));
    esp_mochi_node_unlock_state(node);

    TEST_ASSERT_EQUAL(
        ESP_MOCHI_NODE_CONNECT_SOURCE_KIND_PASSWORD,
    ...
    TEST_ASSERT_EQUAL(
        ESP_MOCHI_NODE_CONNECT_SOURCE_KIND_SHARED_TOKEN,
        token_material.kind);
    ...
    esp_mochi_node_free_connect_material(&password_material);
    esp_mochi_node_free_connect_material(&token_material);
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_destroy(node));
}

TEST_CASE("destroy preserves the caller task notification count", "[esp_mochi_node][destroy]")
{
    reset_mochi_storage();
    reset_transport_state();
...
TEST_CASE("connect kicks challenge ping and disconnect is not dropped while busy", "[esp_mochi_node][transport]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    esp_mochi_node_command_t command = {
        .name = "block",
        .handler = blocking_command_handler,
        .context = &command_ctx,
    };
    TEST_ASSERT_EQUAL(ESP_OK, esp_mochi_node_register_command(node, &command));
    ...
    TEST_ASSERT_TRUE(wait_for_event(ESP_MOCHI_NODE_EVENT_CONNECTED, pdMS_TO_TICKS(1000)));
    ...
    TEST_ASSERT_TRUE(wait_for_event(ESP_MOCHI_NODE_EVENT_DISCONNECTED, pdMS_TO_TICKS(2000)));
...
TEST_CASE("node.invoke.request is ignored until handshake reaches ready", "[esp_mochi_node][protocol]")
{
    reset_mochi_storage();
    reset_transport_state();
...
TEST_CASE("disconnect is rejected while transport start is in progress", "[esp_mochi_node][transport]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    TEST_ASSERT_TRUE(wait_for_event(ESP_MOCHI_NODE_EVENT_CONNECT_FAILED, pdMS_TO_TICKS(1000)));
    TEST_ASSERT_EQUAL(0, s_event_recorder.connected_count);
...
TEST_CASE("disconnect is rejected after connect request send while still connecting", "[esp_mochi_node][transport]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    TEST_ASSERT_EQUAL(0, s_event_recorder.disconnected_count);
    TEST_ASSERT_TRUE(esp_mochi_node_has_saved_session(node));
...
TEST_CASE("clean websocket close keeps local err clear", "[esp_mochi_node][transport]")
{
    reset_mochi_storage();
    reset_transport_state();
    ...
    TEST_ASSERT_TRUE(wait_for_event(ESP_MOCHI_NODE_EVENT_DISCONNECTED, pdMS_TO_TICKS(1000)));
    TEST_ASSERT_EQUAL(ESP_OK, s_event_recorder.local_err);
    TEST_ASSERT_EQUAL(ESP_MOCHI_NODE_DISCONNECTED_REASON_CONNECTION_LOST, s_event_recorder.disconnected_reason);
