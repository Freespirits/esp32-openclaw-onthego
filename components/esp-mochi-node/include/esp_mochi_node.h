...
#define ESP_MOCHI_NODE_MAX_CAPABILITIES CONFIG_ESP_MOCHI_NODE_MAX_CAPABILITIES
/** @brief Maximum number of commands a node can register. */
#define ESP_MOCHI_NODE_MAX_COMMANDS CONFIG_ESP_MOCHI_NODE_MAX_COMMANDS

/* Core Types */

/** @brief Opaque handle for an Mochi Node instance. */
typedef struct esp_mochi_node *esp_mochi_node_handle_t;

/* Event Types */

/** @brief Terminal and maintenance events emitted by the component. */
typedef enum {
    ESP_MOCHI_NODE_EVENT_CONNECTED = 0,  /**< Handshake completed and the session is ready. */
    ESP_MOCHI_NODE_EVENT_CONNECT_FAILED, /**< A connect attempt ended without reaching ready state. */
    ESP_MOCHI_NODE_EVENT_DISCONNECTED,   /**< An established session disconnected or was closed locally. */
} esp_mochi_node_event_t;

/** @brief Failure reasons surfaced on @ref ESP_MOCHI_NODE_EVENT_CONNECT_FAILED. */
typedef enum {
    ESP_MOCHI_NODE_CONNECT_FAILURE_TRANSPORT_START_FAILED = 0,  /**< Transport startup or local connect setup failed. */
    ESP_MOCHI_NODE_CONNECT_FAILURE_CONNECTION_LOST,             /**< The transport dropped before connect completed. */
    ESP_MOCHI_NODE_CONNECT_FAILURE_AUTH_REJECTED,               /**< The gateway rejected the auth material or signature. */
    ESP_MOCHI_NODE_CONNECT_FAILURE_SESSION_FINALIZATION_FAILED, /**< `hello-ok` handling failed after initial auth acceptance. */
} esp_mochi_node_connect_failure_reason_t;

/** @brief Payload for @ref ESP_MOCHI_NODE_EVENT_CONNECT_FAILED. */
typedef struct {
    esp_mochi_node_connect_failure_reason_t reason; /**< High-level failure category. */
...
} esp_mochi_node_connect_failed_event_t;

/** @brief Disconnect reasons surfaced on @ref ESP_MOCHI_NODE_EVENT_DISCONNECTED. */
typedef enum {
    ESP_MOCHI_NODE_DISCONNECTED_REASON_REQUESTED = 0,   /**< Disconnect was requested locally. */
    ESP_MOCHI_NODE_DISCONNECTED_REASON_CONNECTION_LOST, /**< Disconnect followed a transport loss or remote close. */
} esp_mochi_node_disconnected_reason_t;

/** @brief Payload for @ref ESP_MOCHI_NODE_EVENT_DISCONNECTED. */
typedef struct {
    esp_mochi_node_disconnected_reason_t reason; /**< High-level disconnect category. */
    esp_err_t local_err;                            /**< Error code associated with the disconnect, or ESP_OK if none applies. */
} esp_mochi_node_disconnected_event_t;
 ...
 * `esp_mochi_node_request_*()` APIs. It must not call
 * @ref esp_mochi_node_destroy.
 *
 * For each accepted connect request, wait for exactly one terminal outcome:
 * - @ref ESP_MOCHI_NODE_EVENT_CONNECTED
 * - @ref ESP_MOCHI_NODE_EVENT_CONNECT_FAILED
 * - @ref ESP_MOCHI_NODE_EVENT_DISCONNECTED after an accepted disconnect of an
 *   active session
 *
 * `event_data` is:
 * - `NULL` for @ref ESP_MOCHI_NODE_EVENT_CONNECTED
 * - @ref esp_mochi_node_connect_failed_event_t for
 *   @ref ESP_MOCHI_NODE_EVENT_CONNECT_FAILED
 * - @ref esp_mochi_node_disconnected_event_t for
 *   @ref ESP_MOCHI_NODE_EVENT_DISCONNECTED
 *
 * The payload pointer is valid only for the duration of the callback.
 */
typedef void (*esp_mochi_node_event_cb_t)(
 ...
 * Ethernet, PPP, or any other network path; it opens the Mochi WebSocket
...
typedef struct {
    ...
    esp_mochi_node_event_cb_t event_cb; /**< Optional event callback invoked on the component's internal node task. */
    void *event_user_ctx;          /**< Opaque caller context passed back to @p event_cb. */
} esp_mochi_node_config_t;
...
} esp_mochi_node_error_t;

/**
 * @brief Handler for a single advertised Mochi command.
 *
 ...
 */
typedef esp_err_t (*esp_mochi_node_command_handler_t)(
    esp_mochi_node_handle_t node,
    void *context,
...
/** @brief Command registration entry passed to esp_mochi_node_register_command(). */
typedef struct {
    const char *name;                            /**< Command name advertised to the gateway, for example `wifi.status`. */
    esp_mochi_node_command_handler_t handler; /**< Command callback invoked for matching requests. */
    void *context;                               /**< Opaque caller context passed to @p handler. */
} esp_mochi_node_command_t;

/* Connect Input Types */

/** @brief Caller-chosen source for one connect attempt. */
typedef enum {
    ESP_MOCHI_NODE_CONNECT_SOURCE_SAVED_SESSION = 0, /**< Reconnect with the persisted `{ gateway_uri, device_token }` session. */
    ESP_MOCHI_NODE_CONNECT_SOURCE_SETUP_CODE,        /**< Decode and use a setup code that contains the gateway URI and exactly one auth secret. */
    ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_TOKEN,     /**< Connect with an explicit shared gateway token. */
    ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_PASSWORD,  /**< Connect with an explicit shared gateway password. */
    ESP_MOCHI_NODE_CONNECT_SOURCE_NO_AUTH,           /**< Connect to a gateway that intentionally allows unauthenticated node access. */
} esp_mochi_node_connect_source_t;

/**
 * @brief Connect request submitted to @ref esp_mochi_node_request_connect().
 *
 ...
 */
typedef struct {
    esp_mochi_node_connect_source_t source; /**< Caller-chosen source for this one connect attempt. */
...
/**
 * @brief Register a handler for one Mochi command.
 *
 * @param[in] node Node handle.
