...
static const char *TAG = "esp_mochi_node_session";
...
static esp_err_t write_persisted_session_to_storage(const esp_mochi_node_persisted_session_t *update);
...
static void clear_persisted_session_struct(esp_mochi_node_persisted_session_t *session)
{
...
static esp_err_t validate_persisted_session(const esp_mochi_node_persisted_session_t *session)
{
    if (session == NULL) {
...
static esp_err_t clear_invalid_loaded_session(
    nvs_handle_t nvs,
    esp_mochi_node_persisted_session_t *session,
    const char *reason)
{
...
static esp_err_t copy_persisted_session(
    esp_mochi_node_persisted_session_t *dst,
    const esp_mochi_node_persisted_session_t *src)
{
...
static esp_err_t write_persisted_session_to_storage(const esp_mochi_node_persisted_session_t *update)
{
    ...
    if (!esp_mochi_node_persisted_session_is_present(update)) {
...
bool esp_mochi_node_persisted_session_is_present(const esp_mochi_node_persisted_session_t *session)
{
    return session != NULL &&
           session->gateway_uri != NULL &&
...
esp_err_t esp_mochi_node_persisted_session_load(esp_mochi_node_persisted_session_t *session)
{
    if (session == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
void esp_mochi_node_persisted_session_free(esp_mochi_node_persisted_session_t *session)
{
    if (session == NULL) {
        return;
    }
...
esp_err_t esp_mochi_node_persisted_session_store(
    esp_mochi_node_persisted_session_t *session,
    const esp_mochi_node_persisted_session_t *update)
{
    ...
    esp_mochi_node_persisted_session_t copy = {0};
    if (esp_mochi_node_persisted_session_is_present(update)) {
    ...
    if (err != ESP_OK) {
        esp_mochi_node_persisted_session_free(&copy);
        return err;
    }
