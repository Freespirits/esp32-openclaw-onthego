...
static const char *TAG = "esp_mochi_node_identity";
static const char *NVS_NAMESPACE = "mochi";
...
#define ESP_MOCHI_NODE_DEVICE_AUTH_PAYLOAD_V3_FORMAT \
    "v3|%s|%s|%s|%s|%s|%" PRId64 "|%s|%s|%s|%s"

_Static_assert(ESP_MOCHI_NODE_ED25519_SEED_LEN == crypto_sign_SEEDBYTES, "unexpected libsodium seed size");
_Static_assert(
    ESP_MOCHI_NODE_ED25519_PUBLIC_KEY_LEN == crypto_sign_PUBLICKEYBYTES,
    "unexpected libsodium public key size");
_Static_assert(
    ESP_MOCHI_NODE_ED25519_PRIVATE_KEY_LEN == crypto_sign_SECRETKEYBYTES,
...
{
    esp_fill_random(seed, ESP_MOCHI_NODE_ED25519_SEED_LEN);
    ESP_RETURN_ON_ERROR(
        nvs_set_blob(nvs, NVS_KEY_SEED, seed, ESP_MOCHI_NODE_ED25519_SEED_LEN),
        TAG,
...
{
    size_t required = ESP_MOCHI_NODE_ED25519_SEED_LEN;
    esp_err_t err = nvs_get_blob(nvs, NVS_KEY_SEED, seed, &required);
    if (err == ESP_OK && required == ESP_MOCHI_NODE_ED25519_SEED_LEN) {
        *created = false;
        return ESP_OK;
    }
...
{
    if (seed == NULL || seed_len != ESP_MOCHI_NODE_ED25519_SEED_LEN) {
...
esp_err_t esp_mochi_node_identity_load_or_create(esp_mochi_node_identity_t *identity)
{
    ...
    uint8_t digest[32] = {0};
    esp_sha(SHA2_256, identity->public_key, ESP_MOCHI_NODE_ED25519_PUBLIC_KEY_LEN, digest);
    bytes_to_lower_hex(digest, sizeof(digest), identity->device_id, sizeof(identity->device_id));
    err = base64url_encode(
        identity->public_key,
        ESP_MOCHI_NODE_ED25519_PUBLIC_KEY_LEN,
    ...
    char **out_payload)
{
    ...
    int required = snprintf(
        NULL,
        0,
        ESP_MOCHI_NODE_DEVICE_AUTH_PAYLOAD_V3_FORMAT,
        identity->device_id,
        client_id,
        client_mode,
        role,
