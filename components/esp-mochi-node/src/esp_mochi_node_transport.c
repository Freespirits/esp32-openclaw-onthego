...
{
    return node->state != ESP_MOCHI_NODE_INTERNAL_DESTROYING &&
...
static void clear_active_transport_fields_locked(esp_mochi_node_handle_t node)
{
    node->ws = NULL;
    ...
    bool stop_client)
{
    ...
    esp_mochi_node_transport_event_ctx_t *transport_ctx = NULL;
    ...
    esp_mochi_node_handle_t node)
{
    ...
    case ESP_MOCHI_NODE_CONNECT_SOURCE_KIND_SAVED_SESSION:
        gateway_uri =
            esp_mochi_node_trimmed_or_null(node->persisted_session.gateway_uri);
        break;
    ...
    case ESP_MOCHI_NODE_CONNECT_SOURCE_KIND_NO_AUTH:
        gateway_uri =
            esp_mochi_node_trimmed_or_null(node->active_connect_source.gateway_uri);
        break;
    ...
    ESP_RETURN_ON_ERROR(
        esp_mochi_node_validate_tls_preflight(&node->config, gateway_uri),
        ESP_MOCHI_NODE_TAG,
        "TLS preflight failed");

    char *gateway_uri_copy = esp_mochi_node_duplicate_string(gateway_uri);
    ...
    esp_websocket_client_config_t ws_config = {
        ...
        .ping_interval_sec = ESP_MOCHI_NODE_WS_PING_INTERVAL_SEC,
        .pingpong_timeout_sec = ESP_MOCHI_NODE_WS_PINGPONG_TIMEOUT_SEC,
        .keep_alive_enable = true,
        .keep_alive_idle = 5,
        .keep_alive_interval = 5,
        .keep_alive_count = 3,
        .task_prio = 5,
        .task_stack = ESP_MOCHI_NODE_TRANSPORT_TASK_STACK_SIZE,
        .buffer_size = ESP_MOCHI_NODE_TRANSPORT_BUFFER_SIZE,
    ...
    esp_err_t err = node->websocket_client_ops->register_events(
        ...
        transport_ctx);
    if (err != ESP_OK) {
        esp_mochi_node_cleanup_transport_instance(node, false);
        return err;
    }
    ...
    void *event_data)
{
    ...
    esp_mochi_node_transport_event_ctx_t *transport_ctx =
        (esp_mochi_node_transport_event_ctx_t *)handler_args;
    if (transport_ctx == NULL || transport_ctx->node == NULL) {
        return;
    }

    esp_mochi_node_handle_t node = transport_ctx->node;
    esp_websocket_event_data_t *data =
        (esp_websocket_event_data_t *)event_data;

    esp_mochi_node_lock_state(node);
    bool accept = esp_mochi_node_should_accept_callback_transport_id_locked(
        node,
    ...
    case WEBSOCKET_EVENT_CONNECTED: {
        esp_mochi_node_work_message_t message = {
            .type = ESP_MOCHI_NODE_WORK_MSG_WS_CONNECTED,
            .transport_id = transport_ctx->transport_id,
            .local_err = ESP_OK,
        };
        esp_mochi_node_enqueue_work_message_from_callback(node, &message);
        break;
    }
    case WEBSOCKET_EVENT_DISCONNECTED:
    case WEBSOCKET_EVENT_CLOSED: {
        esp_mochi_node_work_message_t message = {
            .type = ESP_MOCHI_NODE_WORK_MSG_WS_DISCONNECTED,
            .transport_id = transport_ctx->transport_id,
    ...
    case WEBSOCKET_EVENT_ERROR: {
        esp_mochi_node_work_message_t message = {
            .type = ESP_MOCHI_NODE_WORK_MSG_WS_ERROR,
            .transport_id = transport_ctx->transport_id,
            .local_err = local_err_from_ws_event(data),
        };
        esp_mochi_node_enqueue_work_message_from_callback(node, &message);
    ...
    case WEBSOCKET_EVENT_DATA:
        ...
        esp_mochi_node_lock_state(node);
        if (!esp_mochi_node_should_accept_callback_transport_id_locked(
                node,
                transport_ctx->transport_id)) {
            esp_mochi_node_unlock_state(node);
            break;
        ...
        if (data->payload_offset == 0) {
            ...
            if (node->rx_buffer == NULL) {
                ...
                esp_mochi_node_unlock_state(node);
                ESP_LOGE(ESP_MOCHI_NODE_TAG, "failed allocating rx buffer");
                break;
            }
        }
        if (node->rx_buffer == NULL ||
        ...
        if (data->payload_offset + data->data_len >= data->payload_len &&
            ...
            esp_mochi_node_work_message_t message = {
                .type = ESP_MOCHI_NODE_WORK_MSG_DATA,
                .transport_id = transport_ctx->transport_id,
                .text = node->rx_buffer,
            };
            node->rx_buffer = NULL;
