...
static bool websocket_send_json(esp_mochi_node_handle_t node, cJSON *root)
{
...
{
    esp_mochi_node_connect_material_t material = {0};

    esp_mochi_node_lock_state(node);
    esp_err_t selection_err =
        esp_mochi_node_resolve_active_connect_material_locked(node, &material);
    ...
    char *payload = NULL;
    esp_err_t err = esp_mochi_node_identity_build_auth_payload_v3(
        &node->identity,
    ...
    if (err != ESP_OK || payload == NULL) {
        ...
        ESP_LOGE(ESP_MOCHI_NODE_TAG, "failed building auth payload");
        return false;
    }

    char signature[ESP_MOCHI_NODE_SIGNATURE_B64_BUFFER_LEN] = {0};
    err = esp_mochi_node_identity_sign_payload(
        &node->identity,
        payload,
        signature,
        sizeof(signature));
    ...
    if (err != ESP_OK) {
        esp_mochi_node_free_connect_material(&material);
        ESP_LOGE(ESP_MOCHI_NODE_TAG, "failed signing auth payload");
        return false;
    }
    ...
    cJSON_AddStringToObject(
        client,
        "version",
        esp_mochi_node_firmware_version());
    ...
    esp_mochi_node_add_registered_string_array(
        params,
        "caps",
        node->capabilities,
        node->capability_count);
    esp_mochi_node_add_registered_command_array(params, "commands", node);
    ...
    esp_mochi_node_internal_state_t state = ESP_MOCHI_NODE_INTERNAL_IDLE;
    esp_mochi_node_lock_state(node);
    state = node->state;
    ready_to_send = node->state == ESP_MOCHI_NODE_INTERNAL_CONNECTING &&
    ...
    ESP_LOGI(
        ...
        esp_mochi_node_connect_source_kind_name(material.kind));
    esp_mochi_node_free_connect_material(&material);
    return true;
}
    ...
    const char *error_message)
{
    ...
    if (!websocket_send_json(node, root)) {
        ESP_LOGW(ESP_MOCHI_NODE_TAG, "failed sending invoke result");
    }
    cJSON_Delete(root);
}
    ...
    esp_mochi_node_persisted_session_t *update)
{
    ...
    update->version = 1;
    update->gateway_uri = esp_mochi_node_duplicate_string(
        esp_mochi_node_trimmed_or_null(node->transport_gateway_uri));
    update->device_token =
        esp_mochi_node_duplicate_string(device_token_text);
    if (update->gateway_uri == NULL || update->device_token == NULL) {
        esp_mochi_node_persisted_session_free(update);
...
static connect_response_finalize_result_t finalize_connect_response_success(
    esp_mochi_node_handle_t node,
    const esp_mochi_node_persisted_session_t *update)
{
    connect_response_finalize_result_t result = {
        .outcome = CONNECT_RESPONSE_OUTCOME_IGNORE,
        .err = ESP_OK,
        .state = ESP_MOCHI_NODE_INTERNAL_IDLE,
    };

    esp_mochi_node_lock_state(node);
    result.state = node->state;
    if (node->state != ESP_MOCHI_NODE_INTERNAL_CONNECTING ||
        node->pending_connect_id[0] == '\0') {
        esp_mochi_node_unlock_state(node);
        return result;
    }

    result.err = esp_mochi_node_persisted_session_store(
        &node->persisted_session,
        update);
    if (result.err != ESP_OK) {
        result.outcome = CONNECT_RESPONSE_OUTCOME_CONNECT_FAILED;
    } else {
        node->state = ESP_MOCHI_NODE_INTERNAL_READY;
    ...
    const connect_response_finalize_result_t *result)
{
    ...
    case CONNECT_RESPONSE_OUTCOME_IGNORE:
        ESP_LOGW(
            ...
            esp_mochi_node_internal_state_name(result->state));
        break;
    ...
    case CONNECT_RESPONSE_OUTCOME_CONNECTED:
        esp_mochi_node_emit_connected(node);
        ESP_LOGI(
            ESP_MOCHI_NODE_TAG,
            "Mochi gateway handshake complete");
        break;
    default:
        break;
    }
    ...
    cJSON *root)
{
    ...
    if (cJSON_IsTrue(ok)) {
        ...
        const char *device_token_text = cJSON_IsString(device_token)
            ? esp_mochi_node_trimmed_or_null(device_token->valuestring)
            : NULL;
        ...
        esp_mochi_node_persisted_session_t update = {0};
    ...
    cJSON *payload)
{
    ...
    esp_mochi_node_unlock_state(node);
    if (!accept_challenge) {
        ESP_LOGW(ESP_MOCHI_NODE_TAG, "ignoring unexpected connect.challenge");
        return;
    }
    ...
    if (!cJSON_IsString(nonce) || nonce->valuestring == NULL ||
        ...
        ESP_LOGW(ESP_MOCHI_NODE_TAG, "connect.challenge missing nonce");
        return;
    }
    ...
    cJSON *payload)
{
    ...
    esp_err_t err = esp_mochi_node_dispatch_command(
        node,
        command->valuestring,
    ...
    const char *text)
{
    ...
    if (root == NULL) {
        ESP_LOGW(ESP_MOCHI_NODE_TAG, "invalid gateway JSON frame");
        return;
    }
    ...
    if (strcmp(type->valuestring, "event") == 0) {
        ...
        if (cJSON_IsString(event) && event->valuestring != NULL) {
            if (strcmp(event->valuestring, "connect.challenge") == 0 &&
                ...
                ESP_LOGI(ESP_MOCHI_NODE_TAG, "received connect.challenge");
                handle_connect_challenge(node, payload);
            } else if (
                strcmp(event->valuestring, "node.invoke.request") == 0 &&
                cJSON_IsObject(payload)) {
                esp_mochi_node_lock_state(node);
                bool ready = node->state == ESP_MOCHI_NODE_INTERNAL_READY;
                esp_mochi_node_unlock_state(node);
                if (ready) {
                ...
                esp_mochi_node_lock_state(node);
                bool connecting =
                    esp_mochi_node_state_is_connecting(node->state);
                esp_mochi_node_unlock_state(node);
                if (connecting) {
                    ESP_LOGD(
                        ESP_MOCHI_NODE_TAG,
