...
`esp-mochi-node` is the ESP-IDF component package in this repository for
running an ESP32 application as an Mochi Node over WebSocket.
The public C API is declared in
[esp_mochi_node.h](./include/esp_mochi_node.h) and uses the
`esp_mochi_node_*` prefix.

The component provides:

- Device identity generation and persistence
- Mochi `connect.challenge` signing and `connect` request construction
...
- ESP-IDF `5.x`
- a board that can reach the Mochi Gateway
...
- Building and signing the Mochi `connect` request
...
- `esp_mochi_node_config_init_default()`
- `esp_mochi_node_create()`
- `esp_mochi_node_destroy()`

### Registration

- `esp_mochi_node_register_capability()`
- `esp_mochi_node_register_command()`

### Async Control

- `esp_mochi_node_request_connect()`
- `esp_mochi_node_request_disconnect()`

### Inspection

- `esp_mochi_node_get_device_id()`
- `esp_mochi_node_has_saved_session()`
...
3. Initialize `esp_mochi_node_config_t` with
  `esp_mochi_node_config_init_default()`.
4. Create the node with `esp_mochi_node_create()`.
5. Register capabilities and commands before the first accepted connect request.
6. Submit one connect request with `esp_mochi_node_request_connect()`.
7. Wait for a terminal event before submitting the next control request.
8. Destroy the node with `esp_mochi_node_destroy()` when finished.
    ...
    void *user_ctx)
{
    ...
    if (event == ESP_MOCHI_NODE_EVENT_CONNECT_FAILED) {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(esp_mochi_node_create(&config, &node));
    ESP_ERROR_CHECK(esp_mochi_node_register_capability(node, "device"));

    esp_mochi_node_command_t cmd = {
        .name = "device.info",
        .handler = handle_device_info,
    };
    ESP_ERROR_CHECK(esp_mochi_node_register_command(node, &cmd));

    esp_mochi_node_connect_request_t request = {
        .source = ESP_MOCHI_NODE_CONNECT_SOURCE_SETUP_CODE,
        .gateway_uri = NULL,
        .value = setup_code,
    };
    ESP_ERROR_CHECK(esp_mochi_node_request_connect(node, &request));
}
```

### Configuration And Defaults

`esp_mochi_node_config_init_default()` sets:

- `display_name = "Mochi ESP32"`
- `platform = "esp32"`
- `device_family = "ESP32"`
- `client_id = "node-host"`
- `client_mode = "node"`
...
```c
ESP_ERROR_CHECK(esp_mochi_node_register_capability(node, "display"));
...
`ESP_MOCHI_NODE_MAX_CAPABILITIES`, `ESP_MOCHI_NODE_MAX_COMMANDS`,
the internal work-queue length, and the component/WebSocket task and buffer
sizes. These can be tuned in `menuconfig` under
`Component config -> ESP Mochi Node`.

Current `menuconfig` options and defaults:

- `CONFIG_ESP_MOCHI_NODE_MAX_CAPABILITIES` = `16`
- `CONFIG_ESP_MOCHI_NODE_MAX_COMMANDS` = `32`
- `CONFIG_ESP_MOCHI_NODE_WORK_QUEUE_LENGTH` = `32`
- `CONFIG_ESP_MOCHI_NODE_TASK_STACK_SIZE` = `8192`
- `CONFIG_ESP_MOCHI_NODE_TRANSPORT_TASK_STACK_SIZE` = `8192`
- `CONFIG_ESP_MOCHI_NODE_TRANSPORT_BUFFER_SIZE` = `2048`
...
- `ESP_MOCHI_NODE_CONNECT_SOURCE_SAVED_SESSION` is valid only when a saved
reconnect session is present
Applications can check saved-session availability with
`esp_mochi_node_has_saved_session()` before submitting that request.
- explicit connect requests are valid only when no session is active and no
  connect or disconnect request is in flight
- `esp_mochi_node_request_disconnect()` is valid only while the node is ready
...
- `ESP_MOCHI_NODE_CONNECT_SOURCE_SAVED_SESSION`
- `ESP_MOCHI_NODE_CONNECT_SOURCE_SETUP_CODE`
- `ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_TOKEN`
- `ESP_MOCHI_NODE_CONNECT_SOURCE_GATEWAY_PASSWORD`
- `ESP_MOCHI_NODE_CONNECT_SOURCE_NO_AUTH`

Field requirements for `esp_mochi_node_connect_request_t`:
...
sequenceDiagram
    ...
    participant Node as esp_mochi_node
    participant NVS as NVS
    participant GW as Mochi Gateway

    App->>Node: esp_mochi_node_create()
    Node->>NVS: load or create device_seed
    Node->>NVS: load saved reconnect session
    App->>Node: register capabilities and commands
    App->>Node: esp_mochi_node_request_connect(SETUP_CODE)
    ...
    alt hello-ok with auth.deviceToken
        ...
        Node-->>App: ESP_MOCHI_NODE_EVENT_CONNECTED
    else auth rejected or finalization fails
        GW-->>Node: error or incomplete hello-ok
        Node-->>App: ESP_MOCHI_NODE_EVENT_CONNECT_FAILED
    end
...
The component emits these events through `esp_mochi_node_event_cb_t`:

- `ESP_MOCHI_NODE_EVENT_CONNECTED`
- `ESP_MOCHI_NODE_EVENT_CONNECT_FAILED`
- `ESP_MOCHI_NODE_EVENT_DISCONNECTED`

`ESP_MOCHI_NODE_EVENT_CONNECT_FAILED` carries
`esp_mochi_node_connect_failed_event_t` with:

- `ESP_MOCHI_NODE_CONNECT_FAILURE_TRANSPORT_START_FAILED`
- `ESP_MOCHI_NODE_CONNECT_FAILURE_CONNECTION_LOST`
- `ESP_MOCHI_NODE_CONNECT_FAILURE_AUTH_REJECTED`
- `ESP_MOCHI_NODE_CONNECT_FAILURE_SESSION_FINALIZATION_FAILED`

`ESP_MOCHI_NODE_EVENT_DISCONNECTED` carries
`esp_mochi_node_disconnected_event_t` with:
...
- callbacks must not call `esp_mochi_node_destroy()`

### Stored State

The component stores internal state in NVS namespace `mochi`.
...
```json
{
  ...
  "params": {
    ...
    "userAgent": "esp-mochi-node/1.0.0",
    "locale": "en-US",
    "device": {
      "id": "<device-id>",
...
```json
{
  ...
  "payload": {
    ...
    "paramsJSON": "{\"heading\":\"Mochi\",\"text\":\"Hello from the gateway.\"}"
  }
}
```
...
```json
{
  ...
  "params": {
    ...
    "nodeId": "<node-id>",
    "ok": true,
    "payloadJSON": "{\"heading\":\"Mochi\",\"text\":\"Hello from the gateway.\",\"renderCount\":1}"
  }
}
```
...
Useful fields in `esp_mochi_node_config_t`:
...
[examples/common/esp_mochi_node_example_saved_session_reconnect.c](../../examples/common/esp_mochi_node_example_saved_session_reconnect.c).
That helper:

- waits for Wi-Fi to be online
- checks `esp_mochi_node_has_saved_session()`
...
[components/esp-mochi-node/test_apps](./test_apps/README.md).
